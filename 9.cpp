...
public:
    int widthOfBinaryTree(TreeNode* root) {
      ...
      unsigned long long a=0;
        ...
        {
            auto s=q.size();
             a=max(a,q.back().second-q.front().second); // the distance between last and first index of that particular level , and calculated the max
            
            for(int i=0;i<s;i++) // gets the next level ready to check the distance
            {
